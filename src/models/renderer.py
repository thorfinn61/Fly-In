...
class Renderer:
    def __init__(self, simulation) -> None:
        ...
        # --- TOP DASHBOARD PANEL ---
        self.panel = tk.Frame(self.root, bg=self.theme["header_bg"], height=80)
        self.panel.pack(fill=tk.X, side=tk.TOP)
        self.panel.pack_propagate(False)
        
        # Subtle Separator
        tk.Frame(self.root, bg=self.theme["border"], height=1).pack(fill=tk.X, side=tk.TOP)
        ...
        btn_style = {"font": ("Segoe UI", 10, "bold"), "fg": self.theme["text"], "bg": self.theme["btn_bg"], 
                     "activebackground": self.theme["btn_hover"], "activeforeground": "white", 
        ...
        tk.Label(speed_frame, text="SPEED CAP", font=("Segoe UI", 8, "bold"), bg=self.theme["header_bg"], fg=self.theme["subtext"]).pack()
        self.speed_slider = tk.Scale(speed_frame, from_=60, to=2, orient=tk.HORIZONTAL, bg=self.theme["header_bg"], fg=self.theme["text"], 
                                     highlightthickness=0, showvalue=0, length=100, troughcolor=self.theme["card_bg"], width=8, bd=0, sliderrelief="flat")
        self.speed_slider.set(20)
        self.speed_slider.pack()

        self.btn_play = tk.Button(ctrl_frame, text="▶ PLAY", command=self.cmd_toggle, **btn_style)
        self.btn_play.config(bg=self.theme["drone_flight"], fg="#0B1120", activebackground="#0EA5E9", activeforeground="#0B1120")
        self.btn_play.pack(side=tk.LEFT, padx=5, fill=tk.Y)
        
        self.btn_restart = tk.Button(ctrl_frame, text="↻ RESET", command=self.cmd_restart, **btn_style)
        self.btn_restart.pack(side=tk.LEFT, padx=5, fill=tk.Y)

        # --- CANVAS SECTION ---
        self.canvas_frame = tk.Frame(self.root, bg=self.theme["bg"])
        self.canvas_frame.pack(fill=tk.BOTH, expand=True)
        self.canvas = tk.Canvas(self.canvas_frame, bg=self.theme["bg"], highlightthickness=0)
    ...
    def setup_from_graph(self):
        self.canvas.delete("all")
        self.drone_shapes = {}
        self.prev_positions = {d.id: d.current_zone for d in self.sim.drones}
        ...
        for conn in self.sim.graph.connections:
            z1, z2 = self.sim.graph.zones[conn.zone1], self.sim.graph.zones[conn.zone2]
            x1, y1 = self.get_coords(z1.x, z1.y)
            x2, y2 = self.get_coords(z2.x, z2.y)
            self.canvas.create_line(x1, y1, x2, y2, fill=self.theme["bg"], width=8, capstyle=tk.ROUND) 
            self.canvas.create_line(x1, y1, x2, y2, fill=self.theme["line"], width=2)
        ...
        for zone in self.sim.graph.zones.values():
            ...
            self.canvas.create_oval(x - self.node_radius, y - self.node_radius, 
                                    ...
                                    fill=base_col, outline=outline_col, width=2)
            
            cap = "∞" if zone.max_drones > 900 else str(zone.max_drones)
            self.canvas.create_text(x, y - self.node_radius - 14, text=zone.name, fill=self.theme["text"], font=("Segoe UI", 9, "bold"))
            self.canvas.create_text(x, y + self.node_radius + 14, text=f"Max: {cap}", fill=self.theme["subtext"], font=("Segoe UI", 8))
    ...
    def get_coords(self, x, y):
        return self.padding + (x - self.min_x) * self.scale_x, self.padding + (y - self.min_y) * self.scale_y

    def get_offsets(self, drones_list):
        opts = {}
        n = len(drones_list)
        if n == 0: return opts
        if n == 1: return {drones_list[0].id: (0, 0)}
        
        radius = min(max(8, n * 2), self.node_radius - 6)
        for i, d in enumerate(drones_list):
            angle = i * (math.pi * 2 / n) - (math.pi / 2)
            opts[d.id] = (math.cos(angle) * radius, math.sin(angle) * radius)
        return opts
        
    def cmd_toggle(self):
        self.sim.is_running = not self.sim.is_running
        if self.sim.is_running:
            self.btn_play.config(text="⏸ PAUSE", bg=self.theme["btn_bg"], fg=self.theme["text"])
            self.pulse()
        else:
            self.btn_play.config(text="▶ PLAY", bg=self.theme["drone_flight"], fg="#0B1120")
    ...
    def render(self, turn: int, initial=False):
        ...
        starts = {z: [] for z in self.sim.graph.zones}
        ends = {z: [] for z in self.sim.graph.zones}
        ...
        frames = 1 if initial else max(1, self.speed_slider.get())
        ...
        for f in range(frames + 1):
            ...
            for d in self.sim.drones:
                ...
                ez = d.current_zone
                sx, sy = self.get_coords(self.sim.graph.zones[sz].x, self.sim.graph.zones[sz].y)
                ex, ey = self.get_coords(self.sim.graph.zones[ez].x, self.sim.graph.zones[ez].y)
                
                sox, soy = start_o.get(d.id, (0, 0))
                eox, eoy = end_o.get(d.id, (0, 0))
                
                cx = (sx + sox) * (1 - t) + (ex + eox) * t
                cy = (sy + soy) * (1 - t) + (ey + eoy) * t
                
                col = self.theme["drone_waiting"]
                if d.status == "in_flight": col = self.theme["drone_flight"]
                elif d.status == "arrived": col = self.theme["drone_arrived"]
